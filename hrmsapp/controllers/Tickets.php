...
class Tickets extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('19',role_resource_ids())) {
			if(!empty($session)){ 
    ...
    public function ticket_list()
     {
		...
		// employee full name
		$employee_name = change_fletter_caps($employee[0]->first_name);
		// priority
		if($r->ticket_priority==1): $priority = 'Low'; elseif($r->ticket_priority==2): $priority = 'Medium'; elseif($r->ticket_priority==3): $priority = 'High'; elseif($r->ticket_priority==4): $priority = 'Critical';  endif;
		 ...
		 // status
		 if($r->ticket_status==1): $status = '<span class="label label-success">Open</span>'; elseif($r->ticket_status==2): $status = '<span class="label label-warning">Closed</span>'; endif;
		...
		$data[] = array(
			...
			$status,
			$_created_at,
			'<ul class="icons-list"><li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="icon-menu9"></i></a><ul class="dropdown-menu dropdown-menu-right"><li><a href="#" data-toggle="modal" data-target=".edit-modal-data"  data-ticket_id="'. $r->ticket_id . '"><i class="icon-pencil7"></i> Edit</a></li><li><a href="'.site_url().'tickets/details/'.$r->ticket_id.'"><i class="icon-eye4"></i> View</a></li><li><a class="delete" href="#" data-record-id="'. $r->ticket_id . '"><i class="icon-trash"></i> Delete</a></li></ul></li></ul>',
		);
      }
	...
	public function add_ticket() {
		...
		userlogs('Tickets','Ticket Added For Employee',$this->input->post('employee_id'));
		if ($result == TRUE) {
	...
	public function update() {
		...
		userlogs('Tickets','Ticket Updated For Employee',$this->input->post('employee_id'));
		if ($result == TRUE) {
	...
	public function update_status() {
		...
		$get_user_id=$this->Employees_model->get_user_action_id($id,'ticket_id','xin_support_tickets');
		userlogs('Tickets','Ticket Status Updated',$get_user_id);
		if ($result == TRUE) {
	...
	public function delete() {
		if($this->input->post('is_ajax') == 2) {
			...
			$id = $this->uri->segment(3);
			$get_user_id=$this->Employees_model->get_user_action_id($id,'ticket_id','xin_support_tickets');
		    userlogs('Tickets','Ticket Deleted',$get_user_id);
