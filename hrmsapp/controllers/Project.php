...
class Project extends MY_Controller {
	...
	public function index()
     {
		...
		if(in_array('7',role_resource_ids())) {
    ...
    public function project_list()
     {
		...
		$pbar='<div class="progress content-group-sm">
									<div class="progress-bar '.$progress_class.' progress-bar-striped" style="width: '.$r->project_progress.'%">
										<span>'.$r->project_progress.'%</span>
									</div>
								</div>';	
		//status
		if($r->status == 0) {
			...
			foreach(explode(',',$r->assigned_to) as $desig_id) {
				...
				$assigned_name = $assigned_to[0]->first_name.' '.$assigned_to[0]->last_name;
				 if($assigned_to[0]->profile_picture!='' && $assigned_to[0]->profile_picture!='no file') {
                    ...
                    $ol .= '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="'.$assigned_name.'"><span class="avatar box-32"><img src="'.$de_file.'" class="img-circle" style="width:100%;" alt=""></span></a>';
                    }
			 }
			 $ol .= '';
		}
		...
		$data[] = array(
			...
			$pbar,
			$ol,
			'<ul class="icons-list"><li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="icon-menu9"></i></a><ul class="dropdown-menu dropdown-menu-right"><li><a href="#" data-toggle="modal" data-target=".edit-modal-data"  data-project_id="'. $r->project_id . '"><i class="icon-pencil7"></i> Edit</a></li><li><a class="delete" href="#" data-record-id="'. $r->project_id . '"><i class="icon-trash"></i> Delete</a></li></ul></li></ul>',
		);
      }
	...
	public function update() {
		...
		userlogs('Projects','Projects Edited',count($this->input->post('assigned_to')));// count of employees
		if ($result == TRUE) {
	...
	public function update_status() {
		...
		$result = $this->Project_model->update_record($data,$id);		
		userlogs('Projects','Projects Status Updated','');// count of employees
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_success_update_project');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
	...
	public function delete() {
		...
		$Return = array('result'=>'', 'error'=>'');
		$id = $this->uri->segment(3);
		userlogs('Projects','Project Deleted','');// count of employees
