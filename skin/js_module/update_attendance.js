...
var department_id=$('#department_id_val').val();
	...
	$.ajax({
		type: "POST",
		url: e.target.action,
		data: obj.serialize()+"&is_ajax=1&add_type=add_manual_attendance&form="+action,
		cache: false,
		success: function (JSON) {
			//$('#show_mail').html(JSON.message);
			if (JSON.error != '') {
...
// Handle click on "Select all" control
   ...
   $('.content').on('click', '.change_schedule', function(){
	   ...
	   if(len>0){
		...
		var modal = $(this);
	    $.ajax({
		url : site_url+"timesheet/read_manual_attendance/",  
		type: "GET",
		data: 'jd=1&is_ajax=1&mode=modal&data=manual_attendance&manual_attendance_id='+arr,
		success: function (response) {			      
			if(response) {
...
swal({
	    ...
	    $.ajax({
		url : site_url+"timesheet/delete_manual_attendance/",  
		type: "POST",
		data: 'jd=1&is_ajax=1&type=delete_manual_attendance&manual_attendance_id='+arr,
		success: function (JSON) {			      
			if (JSON.error != '') {
							toastr.error(JSON.error);
						} else {
    ...
    $('.content').on('click', '.status_schedule', function(){
	   var change_status=$(this).attr('rel');
	   var len = $(".bs_styled:checked").length;
	   var xin_shift_table = $('#xin_shift_table').DataTable();
	    var arr = [];
		xin_shift_table.$('input.bs_styled:checkbox:checked').each(function () {
...
swal({
	    ...
	    $.ajax({
		url : site_url+"timesheet/status_manual_attendance/",  
		type: "POST",
		data: 'jd=1&is_ajax=1&type=status_manual_attendance&manual_attendance_id='+arr+'&change_status='+change_status,
		success: function (JSON) {			      
			if (JSON.error != '') {
							toastr.error(JSON.error);
						} else {
 ...
 if((location_id!=null)){ // && (department_id!=null)
  ...
  $.extend( $.fn.dataTable.defaults, {
	...
	var datatablescroll = $('.datatablescroll').DataTable({
		...
		// scrollY: 300,
		"ajax": {
			url : site_url+"timesheet/employee_list_by_location_department_for_manual/?location_id="+location_id+"&department_id="+department_id,
			type : 'GET'
...
function shift_schedule_list(){
		...
		var ob_type=$('#ob_type_filter').val();	
	$.extend( $.fn.dataTable.defaults, {
        autoWidth: false,
        dom: '<"datatable-header"fBl><"datatable-scroll-wrap"t><"datatable-footer"ip>',
        language: {
	 ...
	 var xin_shift_table = $('#xin_shift_table').DataTable({
		...
		//paging: false,
		"ajax": {
			url : site_url+"timesheet/employee_manual_attendance_lists/?&user_id="+user_id+"&department_value="+department_value+"&ob_type="+ob_type+"&location_value="+location_value+"&month_year="+month_year,
			type : 'GET'
		},"fnDrawCallback": function(settings){
		$('[data-toggle="tooltip"]').tooltip();          
		},'columnDefs': [{
	  ...
	  //read_user_info_manual_attendance_shift
